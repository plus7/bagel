...
xpidl_process_idl(char *filename, IncludePathEntry *include_path,
                  FILE* fd, backendFactory factory)
{
    char *tmp;
    IDL_tree top;
    TreeState state;
...
GSList*
xpidl_get_include(char *filename, IncludePathEntry *include_path)
{
    IDL_tree top;
    IDL_ns ns;
    int rv;
    ...
    return callback_state.base_includes;
}
...
static GSList*
append_file(GSList *slist, char* fullpath, char* filename)
{
    IncludeData *data;
    char*   tmp;
    
    data = xpidl_malloc(sizeof(IncludeData));
    if (data == NULL) return slist;
    data->FullPath = NULL;
    data->FileName = NULL;
    data->Includes = NULL;
    
    tmp = xpidl_strdup(fullpath);
    if (tmp == NULL) goto FAIL;
    data->FullPath = tmp;
    tmp = xpidl_strdup(filename);
    if (tmp == NULL) goto FAIL;
    data->FileName = tmp;
...
GSList*
xpidlpas_append_process_files(GSList *slist, const char* filename)
{
    ...
    if (hFind == INVALID_HANDLE_VALUE) {
        if (enable_warnings)
            fprintf(stderr, "Warning: '%s%' is not found.\n", filename);
        return slist;
    }
    do {
        _makepath(path, drv, dir, fData.cFileName, NULL);
        slist = append_file(slist, path, fData.cFileName);
...
static gint
IncludeCompare(gconstpointer a, gconstpointer b)
{
    const IncludeData* A = a;
    const IncludeData* B = b;
    guint len_a = g_slist_length(A->Includes);
    guint len_b = g_slist_length(B->Includes);
    guint i;
    
    for (i=0; i<len_a; i++) {
        char *file;
        
        file = (char *)g_slist_nth_data(A->Includes, i);
        if (strcmp(file, B->FileName) == 0) {
            if (verbose_mode) {
                fprintf(stderr, "'%s' includes '%s'\n", A->FileName, B->FileName);
            }
            return 1;
        }
    }
    ...
    for (i=0; i<len_b; i++) {
        ...
        file = (char *)g_slist_nth_data(B->Includes, i);
        if (strcmp(file, A->FileName) == 0) {
            if (verbose_mode) {
                fprintf(stderr, "'%s' is included in '%s'\n", A->FileName, B->FileName);
            }
            return -1;
        }
    }
...
static void
CheckInclude(gpointer data, gpointer user_data)
{
    IncludeData *iData = data;
    IncludePathEntry *include_path = user_data;
    GSList *Includes = NULL;
    
    Includes = xpidl_get_include(iData->FullPath, include_path);
    iData->Includes = Includes;
}

GSList*
xpidlpas_sort(GSList *slist)
{
    GSList *top, *next;
    gpointer tdata, ndata;
    gint    i, n;
    ...
    if (top->next != NULL) {
        ...
        next = top;
        for (next = top; next!=NULL; next = g_slist_next(next)) {
            if (IncludeCompare(top->data, next->data) > 0) {
                if (verbose_mode) {
                    IncludeData *n, *t;
                    n = next->data; t = top->data;
                    fprintf(stderr, "Switching %s and %s\n", t->FileName, n->FileName);
                }
                ndata = next->data;
                top = g_slist_remove(top, ndata);
                top = g_slist_prepend(top, ndata);
                next = top;
            }
        }
        top->next = xpidlpas_sort(top->next);
    }
    return top;
}

GSList*
xpidlpas_sort_files(GSList *slist, IncludePathEntry *include_path)
{
    if (verbose_mode)
        fputs("Search Include Files ...\n", stderr);
    g_slist_foreach(slist, CheckInclude, include_path);
    
    if (verbose_mode)
        fputs("Sorting ...\n", stderr);
        
    return xpidlpas_sort(slist);
}

static void
PrintFullPath(gpointer data, gpointer user_data)
{
    IncludeData *iData = data;
    printf ("%s\n", iData->FullPath);
}
...
static void
ProcessIDLFile(gpointer data, gpointer user_data)
{
    IncludeData* iData = data;
    ProcessData* pData = user_data;
    
    xpidl_process_idl(iData->FullPath, pData->include_path, pData->file, pData->factory);
}

static void
xpidl_process_inc(const char* filename, FILE* file)
{
    char *inc_file;
    char *ptr;
    FILE *in_file;
    
    inc_file = xpidl_strdup(filename);
    if (!inc_file) return;
    for(ptr = inc_file; *ptr; ptr++);
    for(; ptr>=inc_file && *ptr!='.'; ptr--);
    if(ptr<inc_file) goto end1;
    strcpy(ptr, ".inc");
    
    in_file = fopen(inc_file, "rt");
    if (!in_file) goto end1;
    while(!feof(in_file)) {
        char buffer[256];
        fgets(buffer, 256, in_file);
        fputs(buffer, file);
    }
    fclose(in_file);
...
static void
ProcessIncFile(gpointer data, gpointer user_data)
{
    IncludeData* iData = data;
    ProcessData* pData = user_data;
    
    xpidl_process_inc(iData->FullPath, pData->file);
}

gboolean
xpidlpas_process_files(GSList *slist, IncludePathEntry *include_path,
                       char *file_basename, ModeData *mode)
{
    FILE *file;
    ProcessData pData;
    char *outname, *real_outname;
    char *basename;
    
    outname = g_strdup_printf("%s.pas", file_basename);
    file = fopen(outname, "w");
    if (!file) return FALSE;
    
    pData.file = file;
    pData.include_path = include_path;
    
    basename = strrchr(file_basename, '\\');
    if (!basename)
    ...
    fputs("interface\n\n", file);
    if (uses_units) {
        GSList *unit = uses_units;
        fputs("uses\n  ", file);
        do {
            fputs((const char*)unit->data, file);
            unit = g_slist_next(unit);
            if (unit)
                fputs(", ", file);
        } while(unit);
        fputs(";\n\n", file);
    ...
    fputs("const\n", file);
    pData.factory = xpidl_const_dispatch;
    g_slist_foreach(slist, ProcessIDLFile, &pData);
    
    fputs("type\n", file);
    pData.factory = xpidl_forward_dispatch;
    g_slist_foreach(slist, ProcessIDLFile, &pData);
    
    g_slist_foreach(slist, ProcessIncFile, &pData);
    
    pData.factory = xpidl_typedef_dispatch;
    g_slist_foreach(slist, ProcessIDLFile, &pData);
    
    pData.factory = mode->factory;
    g_slist_foreach(slist, ProcessIDLFile, &pData);
